...
В `Трафик и активность` -> `Просмотр трафика` сначала показывается список клиентов с трафиком за текущий месяц, после выбора клиента можно посмотреть день по часам, неделю по дням, месяц по дням или произвольный период.
