...
В `Трафик и активность` -> `Просмотр трафика` сначала показывается суммарный расход всех клиентов за текущий месяц и список клиентов с месячным трафиком. После выбора клиента можно посмотреть день по часам, неделю по дням, месяц по дням или произвольный период.
...
В `Трафик и активность` можно посмотреть activity-отчёт по клиенту, подозрительную активность, экспортировать метаданные и управлять архивами экспорта.
