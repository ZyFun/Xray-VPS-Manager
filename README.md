...
Табличный вывод в интерактивном терминале подсвечивает каждую вторую строку мягким фоном, чтобы проще читать длинные списки. Если цвета не нужны, можно отключить ANSI-оформление переменной окружения `NO_COLOR=1`.
...
Перед реальным переключением можно выполнить read-only preflight: он импортирует текущие JSON/JSONL во временную SQLite-базу, проверяет её и удаляет временный файл, не меняя настоящий `manager.db`.
На больших журналах активности preflight и cutover могут занимать заметное время. SQLite-файл может быть больше исходных JSON/JSONL, потому что события раскладываются по колонкам, сохраняется `raw_json`, создаются индексы для отчётов и используются служебные страницы базы.
...
Cutover останавливает manager-сервисы, которые могут писать в JSON, отдельно проверяет, что writer units больше не активны, создаёт и проверяет pre-cutover backup, импортирует JSON/JSONL в SQLite, проверяет базу, включает SQLite-чтение/запись, проверяет read-layer и связи SQLite, запускает writer-сервисы обратно, проверяет их состояние и выполняет `xray-test`.
Если на старой установке какого-то manager unit ещё нет, он пропускается с предупреждением; настоящие ошибки `systemctl` остаются фатальными и останавливают cutover.

После переключения можно отдельно проверить, что база готова, оба SQLite-флага включены, read-layer клиентов, трафика, Telegram и журнала активности использует SQLite, runtime-сценарии чтения данных проходят, а в основных связях нет осиротевших строк:
