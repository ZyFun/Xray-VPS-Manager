...
В разделе `Клиенты` есть действие `Продлить подписку`. После него бот покажет кнопки с клиентами, затем попросит отправить число дней. Введённое число передаётся в `xray-client extend-days NAME DAYS`, поэтому сохраняются обычные правила продления: если доступ ещё активен, дни добавляются к текущей будущей дате; если доступ истёк или был бессрочным, продление считается от текущей даты в часовом поясе менеджера.
