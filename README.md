...
Клиентская подписка работает так: пользователь пишет боту `/start`, бот показывает inline-кнопки `Подключить уведомления`, `Статус подписки`, `Получить VLESS-ссылку`, `Статистика трафика`, `Отключить уведомления`, `Помощь`. После кнопки `Подключить уведомления` пользователь отправляет VLESS Reality-ссылку, затем менеджер сопоставляет её с клиентом по UUID, порту и Reality-параметрам `pbk`, `sni`, `sid`, `fp`, `type`, `flow`. Часть ссылки после `#` игнорируется. Напоминания отправляются в 08:00 по часовому поясу менеджера за 5 дней и за 1 день до `expiresAt`.
