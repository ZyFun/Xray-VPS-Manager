...
Если включён только флаг записи, изменения клиентов и Reality-подключений продолжают сохраняться в JSON и дополнительно зеркалятся в SQLite для проверки. Если одновременно включены чтение и запись SQLite, изменения клиентов и подключений пишутся в SQLite как основной источник; старый JSON остаётся rollback/fallback-данными и не обновляется этим сохранением.
