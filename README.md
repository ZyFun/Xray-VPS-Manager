...
Если включён только флаг записи, изменения клиентов, Reality-подключений и трафика продолжают сохраняться в JSON и дополнительно зеркалятся в SQLite для проверки. Если одновременно включены чтение и запись SQLite, изменения клиентов, подключений и трафика пишутся в SQLite как основной источник; старые JSON-файлы остаются rollback/fallback-данными и не обновляются этими сохранениями.
