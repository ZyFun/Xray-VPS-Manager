...
Cutover останавливает manager-сервисы, которые могут писать в JSON, отдельно проверяет, что writer units больше не активны, создаёт и проверяет pre-cutover backup, импортирует JSON/JSONL в SQLite, проверяет базу, включает SQLite-чтение/запись, проверяет read-layer и связи SQLite, запускает writer-сервисы обратно и выполняет `xray-test`.
