...
Каскады хранятся как именованные Xray outbounds с tag `cascade-{name}`. Имя должно состоять из латинских букв, цифр, `-` или `_`, до 32 символов; первый символ должен быть буквой или цифрой. Старый tag `cascade-upstream` остаётся совместимым и считается каскадом с именем `upstream`.
...
Добавить или заменить каскад интерактивно. Команда спросит имя каскада, затем VLESS-ссылку, создаст tag `cascade-{name}` и сразу сделает этот каскад активным:

```bash
xray-set-cascade
```

Добавить или заменить каскад с заранее заданным именем:
