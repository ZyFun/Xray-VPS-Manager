...
Добавить или заменить каскад интерактивно. Команда спросит имя каскада, страну, затем VLESS-ссылку, создаст tag `cascade-{name}` и сразу сделает этот каскад активным:

```bash
xray-set-cascade
```

Добавить или заменить каскад с заранее заданным именем и, при необходимости, страной:
