...
11 Выбрать GeoIP-регион предупреждений
12 Отключить GeoIP-предупреждения
...
В `Журнал активности` можно включить или отключить только парсер access log, посмотреть отчёты по клиентам, экспортировать метаданные, удалить старые архивы экспорта, изменить срок хранения детальных событий и отдельно выбрать GeoIP-регион для предупреждений о split tunneling.
...
`xray-activity suspicious` - это отдельная сводка по найденным поводам для внимания. Сейчас туда попадают `smtp`, `blocked/torrent`, `geoip:CODE`, всплеск `1000+` событий за час, `500+` уникальных host за период или `20+` уникальных портов за период.
