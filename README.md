...
Если SQLite-чтение и SQLite-запись уже включены, повторный запуск `cutover` не импортирует старые JSON/JSONL ещё раз. В этом случае команда проверяет текущую SQLite-базу и выполняет `xray-test`; если база отсутствует или не готова, команда завершится явной ошибкой.
