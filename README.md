...
Клиентская подписка работает так: пользователь пишет боту `/start`, бот показывает inline-кнопки `Подключить уведомления`, `Статус подписки`, `Получить VLESS-ссылку`, `Статистика трафика`, `Отписаться от бота`, `Помощь`. После кнопки `Подключить уведомления` пользователь отправляет VLESS Reality-ссылку, затем менеджер сопоставляет её с клиентом по UUID, порту и Reality-параметрам `pbk`, `sni`, `sid`, `fp`, `type`, `flow`. Часть ссылки после `#` игнорируется. Если пользователь уже подписан, повторный `/start` или обычное сообщение показывают текущую подписку, а кнопка `Подключить уведомления` больше не выводится в его меню. Напоминания отправляются в 08:00 по часовому поясу менеджера за 5 дней и за 1 день до `expiresAt`; для одного клиента, даты доступа и порога напоминание отправляется один раз.
