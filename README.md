...
| Manager Updated | 2026-06-05 09:53 UTC |
+-----------------+----------------------+
```
...
/usr/local/etc/xray/server.env           адрес сервера, порт, SNI, DEST, fingerprint, timezone
...
Создать бэкап и скачать на локальный компьютер:

```bash
xray-backup create
scp root@SERVER_HOST:/root/xray_backups/ИМЯ_АРХИВА.tar.gz ~/Downloads/
```

Через меню `Резервные копии` -> `Создать бэкап и показать команду скачивания` сервер создаст архив, спросит локальную папку и покажет готовую команду `scp`.
Эту команду нужно выполнить в локальном терминале на компьютере, с которого выполняется подключение.
Введи SSH target/user@host вместо `root@SERVER_HOST`.

Загрузить локальный архив обратно на сервер перед восстановлением:

```bash
scp ~/Downloads/ИМЯ_АРХИВА.tar.gz root@SERVER_HOST:/root/xray_backups/
```
