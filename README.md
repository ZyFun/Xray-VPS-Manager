...
| Manager Updated | 2026-06-05 13:50 UTC |
...
12 Выбрать GeoIP-регион предупреждений
13 Отключить GeoIP-предупреждения
...
В `Журнал активности` можно включить или отключить только парсер access log, посмотреть отчёты по клиентам, экспортировать метаданные, удалить старые архивы экспорта, изменить срок хранения детальных событий, настроить пороги suspicious и отдельно выбрать GeoIP-регион для предупреждений о split tunneling.
...
- `geoip:CODE` - IP назначения попал в выбранный GeoIP-регион предупреждений

`xray-activity suspicious` - это отдельная сводка по найденным поводам для внимания. Сейчас туда попадают `smtp`, `blocked/torrent`, `geoip:CODE`, всплеск событий в rolling-окне, слишком много уникальных host за период или слишком много уникальных портов за период.
...
GeoIP-предупреждения для проверки split tunneling включаются отдельно от парсера и по умолчанию отключены. Через меню можно выбрать регион из списка или найти другой регион поиском по названию из списка либо по коду из `geoip.dat`. Если IP назначения попадает в выбранный регион, отчёт `Подозрительная активность` покажет рекомендацию проверить раздельное туннелирование.
...
Включить предупреждения для выбранного региона:

```bash
xray-activity geo-warnings set RU
```

Отключить GeoIP-предупреждения:

```bash
xray-activity geo-warnings disable
```

Проверка работает только по видимому IP назначения. Если в access log сохранён домен, а не IP, GeoIP-регион для такого события не определяется.
...
По умолчанию лимит не установлен. Если дневной лимит исчерпан, клиент отключается до следующего календарного дня. Если месячный лимит исчерпан, клиент отключается до следующего календарного месяца. После сброса периода клиента можно включить обратно той же командой или через меню.
...
Автоматическая проверка выполняется раз в минуту после сохранения статистики `xray-traffic-sync.timer`.
